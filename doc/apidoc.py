...
def format_inheritance_directive(modules, package=None):
    """Create the automodule directive and add the options."""
    submodule_names = ' '.join([makename(package, module) for module in modules])
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    if submods:
        ...
        if not opts.separatemodules and not opts.noinheritance:
            text += format_heading(2, 'Class inheritance diagram')
            text += format_inheritance_directive([makename(subroot, submod) for submod in submods],
                                                  master_package)
            text += '\n'
        ...
        if opts.separatemodules:
            ...
            for submod in submods:
                ...
                if not opts.noheadings:
                    ...
                    filetext = ''
                if not opts.noinheritance:
                    filetext += format_inheritance_directive([makename(subroot, submod)],
                                                             master_package)
                    filetext += '\n'
...
Note: By default this script will not overwrite already created files.""")
    ...
    parser.add_option('-M', '--module-first', action='store_true',
                      ...
                      'documentation')
    parser.add_option('-I', '--no-inheritance', action='store_true',
                      dest='noinheritance',
                      help='Don\'t create inheritance diagrams for the modules')
