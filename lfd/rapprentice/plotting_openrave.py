...
def draw_grid_2d(env, f, mins, maxes, xres = .1, yres = .1, zres = .04, color = (1,1,0,1)):
    ...
    if zres == -1:
        ...
        # zcoarse = np.arange(zmin, zmax, zres)
        # zmax = zcoarse[-1];
        zcoarse = np.ones(len(xcoarse)) * zmax
    xfine = np.linspace(xmin, xmax, nfine)
    yfine = np.linspace(ymin, ymax, nfine)
    zfine = np.linspace(zmin, zmax, nfine)
    ...
    if len(zcoarse) > 1:    
        for x in xcoarse:
            for y in ycoarse:
                xyz = np.zeros((nfine, 2))
                xyz[:,0] = x
                xyz[:,1] = y
                f_xyz = f(xyz)
                f_xyz = np.hstack((f_xyz, zfine))
                lines.append(f_xyz)
    ...
    for y in ycoarse:
        for z in zcoarse:
            ...
            xyz[:,1] = y
            f_xyz = f(xyz)
            f_xyz = np.hstack((f_xyz, np.ones((len(f_xyz), 1)) * z))
            lines.append(f_xyz)
