...
def main():
    ...
    test_robot = BoxRobotSimulationObject("robot", k * [robot_x, robot_y, robot_z], k * [robot_length, robot_width, robot_height], dynamic=True)
    sim.add_objects([test_robot])
    color_robot([test_robot], [0, 0, 1])
    test_scene_state = get_scene_state(env, test_robot, 3, 12)
    ...
    import pdb; pdb.set_trace()
    traj = plan_trajopt_test_linear(env, test_robot, rel_pts, demo, target_pose, plotting=True)

    generate_pc_from_traj(env, test_robot, rave_robot, traj, num_x_points = 8, plot=True)
    # import pdb; pdb.set_trace()
