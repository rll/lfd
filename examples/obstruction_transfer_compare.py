...
def generate_cloud(x_center_pert=0, max_noise=0.02, dim_x=8, dim_y=5):
    ...
    grid = np.array(np.meshgrid(np.linspace(-.2,.2,dim_y), np.linspace(-.3,.3,dim_x))).T.reshape((-1,2))
...
def main():
    ...
    for i in range(5):
        ...
        demo_cloud = generate_cloud(x_center_pert=x_center_pert, dim_x = x[i], dim_y = y[i])
        demo_scene_state = SceneState(demo_cloud, downsample_size=0.00000001)

        demo = Demonstration(demo_name, demo_scene_state, aug_traj)
        demos[demo_name] = demo

        test_scene_state = SceneState(demo_cloud, downsample_size=0.0000001)
        ...
        decomp_aug_traj = reg_and_traj_transferer.transfer(demo, test_scene_state, callback=plot_cb, plotting=False)
        b = datetime.datetime.now()
        decomp_time.append((b-a).total_seconds())

        reg_and_traj_transferer = UnifiedRegistrationAndTrajectoryTransferer(reg_factory, traj_transferer)
        a = datetime.datetime.now()
        unified_aug_traj = reg_and_traj_transferer.transfer(demo, test_scene_state, callback=plot_cb, plotting=False)
        b = datetime.datetime.now()
        unified_time.append((b-a).total_seconds())
