...
def main():
    ...
    env = environment.LfdEnvironment(sim, sim, downsample_size=0.0000001)


    demos = {}
    x_center_pert = 0.0
    demo_name = "demo_{}".format(x_center_pert)
    demo_cloud = generate_cloud(x_center_pert=x_center_pert, dim_x = 20, dim_y=10)
    demo_scene_state = SceneState(demo_cloud, downsample_size=0.0000001)
    ...
    a = datetime.datetime.now()
    unified_aug_traj = reg_and_traj_transferer.transfer(demo, test_scene_state, callback=plot_cb, plotting=True)
    b = datetime.datetime.now()
    unified_time = b-a
